...
const AdminPage = () => {
    ...
    const verifyAuth = async () => {
      try {
        const response = await fetch(
          "https://api.anandtyping.com/verify-auth",
          {
            credentials: "include",
          }
        );
  ...
  const handleLogout = async () => {
    try {
      const response = await fetch("https://api.anandtyping.com/logout", {
        method: "POST",
        credentials: "include",
      });
      if (response.ok) {
      ...
      <div className="w-full max-w-4xl p-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          ...
          <a
            ...
            className="block bg-white p-6 rounded-lg shadow-lg hover:shadow-xl transition duration-300"
          >
            <h2 className="text-xl font-semibold mb-2">Add Mangal Text</h2>
            <p className="text-gray-600">
              Insert and edit Mangal language text.
            </p>
          </a>
          <a
            href="addfiles"
