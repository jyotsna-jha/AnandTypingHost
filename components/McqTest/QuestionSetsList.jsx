...
const QuestionSetsList = () => {
    ...
    const fetchQuestionSets = async () => {
      try {
        const response = await fetch(
          "https://api.anandtyping.com/question-sets"
        );
        if (!response.ok) {
  ...
  const handleDeleteQuestionSet = async (setId) => {
    try {
      const response = await fetch(
        `https://api.anandtyping.com/question-sets/${setId}`,
        {
          method: "DELETE",
          credentials: "include",
        }
  ...
  const handleDeleteQuestion = async (setId, questionId) => {
    try {
      const response = await fetch(
        `https://api.anandtyping.com/question-sets/${setId}/questions/${questionId}`,
        {
          method: "DELETE",
        }
      );
